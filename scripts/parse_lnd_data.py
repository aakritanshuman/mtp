...
def read_file_lnd(filename):
    ...
    with open(filename + ".json") as f:
        ...
        for edge in data["edges"]:
            ...
            # if node1_policy == "null":
            # if n1=="037d706d809b71937dfeab0cc0577018d31bca5a729605529c5fe746924b2562bb":
            if node1_policy==None:
                # print(node1_policy)
                node1_base_fee = 100
                node1_fee_rate = 1
            else:
                node1_base_fee = node1_policy["fee_base_msat"]
                node1_fee_rate = node1_policy["fee_rate_milli_msat"]

            if node2_policy==None:
                # print(node1_policy)
                node2_base_fee = 100
                node2_fee_rate = 1
            else:
                node2_base_fee = node2_policy["fee_base_msat"]
                node2_fee_rate = node2_policy["fee_rate_milli_msat"]


            lnd_graph.add_edge(n1_id, n2_id, capacity=cap, bf1=node1_base_fee, fr1=node1_fee_rate, bf2=node2_base_fee, fr2=node2_fee_rate)
...
for filename in lnd_file_list:
    file_format = "LND" if "lnd" in filename else "c-lightning"
    graph = read_file(LND_FILE_PATH + filename, file_format)
    new_graph = remove_nodes_based_on_degree(graph, 61)
    new_graph_1 = new_graph.copy()
    
    mapping = dict(list(zip(new_graph_1.nodes(), list(range(0, len(new_graph_1))))))
    new_graph_1 = nx.relabel_nodes(new_graph_1, mapping, copy=False)
    # print(new_graph_1.edges())
    print(nx.is_connected(new_graph_1))
    print(nx.is_frozen(new_graph_1))
    

    print(len(sorted(nx.connected_components(new_graph), key = len, reverse=True)))
    print(str(len(new_graph.nodes()))+" "+str(len(new_graph_1.nodes()))+" "+str(len(graph.nodes())))
    # plot_degree_distribution(graph)
    # plot_shortest_path_distribution(graph)



    capacities = nx.get_edge_attributes(new_graph_1, "capacity")
    capacities = [float(str(c))/SAT_TO_EUR for c in list(capacities.values()) if float(str(c))/SAT_TO_EUR > 2.0]
    print(SAT_TO_EUR)
    print(len(capacities))
    # plt.hist(capacities, bins=100, density=True, cumulative=True)
    ...
    nx.write_edgelist(new_graph_1, LND_FILE_PATH + filename + "_reducedsize.edgelist")
